/*
Title: Line - OBB Collision Detection
File Name: main.cpp
Copyright © 2015
...
Description:
This is a collision detection algorithm between a line and an oriented bounding box in 2D.
By determining which side of the line the corners of the OBB fall on, we can tell if they are colliding.
...
Use WASD to move the selected shape, and Q and E to rotate the selected shape.
Spacebar will switch the selected shape.
...
struct OBB
{
	...
	///
	//Generates an oriented bounding box to a specified width and height
	//
	//Parameters:
	//	w:	The width of the AABB (Before scaling)
	//	h: The height of the AABB (before scaling)
	OBB::OBB(const float w, const float h)
	{
		this->width = w;
		this->height = h;
	}
...
bool TestCollision(const struct OBB &obb, const glm::mat4 &OBBTransform, const LineCollider &lineCol, const glm::mat4 &lineTransform)
{
	//Step 1: Get the corners of the AABB in world space.
	glm::vec4 corner1 = OBBTransform * glm::vec4(obb.width / 2.0f, obb.height / 2.0f, 0.0f, 1.0f);		//Top right
	glm::vec4 corner2 = OBBTransform * glm::vec4(-obb.width / 2.0f, obb.height / 2.0f, 0.0f, 1.0f);		//Top left
	glm::vec4 corner3 = OBBTransform * glm::vec4(-obb.width / 2.0f, -obb.height / 2.0f, 0.0f, 1.0f);	//Bottom left
	glm::vec4 corner4 = OBBTransform * glm::vec4(obb.width / 2.0f, -obb.height / 2.0f, 0.0f, 1.0f);		//Bottom right
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		if (key == GLFW_KEY_SPACE)
			selectedShape = selectedShape == line ? square : line;

		//This set of controls are used to move the selectedShape.
		if (key == GLFW_KEY_W)
			selectedShape->modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, movementSpeed, 0.0f)) * selectedShape->modelMatrix;
		if (key == GLFW_KEY_A)
			selectedShape->modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-movementSpeed, 0.0f, 0.0f)) * selectedShape->modelMatrix;
		if (key == GLFW_KEY_S)
			selectedShape->modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -movementSpeed, 0.0f)) * selectedShape->modelMatrix;
		if (key == GLFW_KEY_D)
			selectedShape->modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(movementSpeed, 0.0f, 0.0f)) * selectedShape->modelMatrix;
		...
		if (key == GLFW_KEY_Q)
			selectedShape->modelMatrix = glm::rotate(selectedShape->modelMatrix, rotationSpeed, glm::vec3(0.0f, 0.0f, 1.0f));
		if (key == GLFW_KEY_E)
			selectedShape->modelMatrix = glm::rotate(selectedShape->modelMatrix, -rotationSpeed, glm::vec3(0.0f, 0.0f, 1.0f));
	}
...
void main()
{
	...
	// Creates a window 
	window = glfwCreateWindow(800, 800, "Line - OBB (2D)", nullptr, nullptr);
	...
	squareCollider = OBB(width, height);
	...
	//Print controls
	std::cout << "Controls:\nUse WASD to move the selected shape.\nUse Q and E to rotate the selected shape.\nUse spacebar to swap the selected shape.\n";
